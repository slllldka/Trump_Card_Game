						...
						while (socket.isConnected() && !socket.isClosed()) {
							while (receiveQueue.size() > 0) {
								...
								// wait
									...
									// holepunch fail
									else if(rec.equals("HolePunchFail")) {
										Room room = clientInfo.room;
										String data = "WStart" + room.drawCard();
										for(ClientInfo client : room.players) {
											data += ".";
											data += client.name;
										}
										room.broadcast("HolePunchFail", null);
										room.broadcast(data, null);
									}
									// waiting
									else {
										...
										for(int i=0;i<Math.min(oneCardPlayerList.size(), 4);i++) {
											if(oneCardPlayerList.get(i).name.equals(recArray[0])) {
												nameExist = true;
												break;
											}
										}
										...
										// available name
											...
											System.out.println(recArray[0]);
											System.out.println(recArray[1]);
											clientInfo.name = recArray[0];
											...
											try {
												...
												System.out.println("UDP PORT: " + packet.getPort());
												clientInfo.udp_port = packet.getPort();
												
												byte[] sendbyte = new String("Hello").getBytes();
												packet = new DatagramPacket(sendbyte, sendbyte.length, packet.getAddress(), packet.getPort());
